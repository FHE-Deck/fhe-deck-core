...
void FunctionalBootstrapPublicKey::bootstrap(LWECT& lwe_ct_out, std::shared_ptr<FunctionSpecification> acc_in, const LWECT& lwe_ct_in){      
    ...
    m_key_switch_key->lwe_to_lwe_key_switch(lwe_c, lwe_ct_in);   
    ...
    m_blind_rotation_key->blind_rotate(*br_out->accumulator, lwe_c, std::static_pointer_cast<VectorCTAccumulator>(acc_in));
    // 4) Sample Extract    
...
std::vector<LWECT> FunctionalBootstrapPublicKey::bootstrap(std::vector<std::shared_ptr<FunctionSpecification>> acc_in_vec, const LWECT& lwe_ct_in, const PlaintextEncoding &output_encoding){   
    ...
    m_ms_from_keyswitch_to_par.switch_modulus(lwe_c, lwe_c); 
    // 3) Blind rotate       
    std::shared_ptr<VectorCTAccumulator> acc_one(m_prepared_acc_builder->get_acc_one(output_encoding)); 
    std::shared_ptr<BlindRotateOutput> br_out(m_blind_rotate_output_builder->build()); 
...
bool FunctionalBootstrapPublicKey::is_amortizable()const{
    return m_is_full_domain_bootstrap_function_amortizable;
}
