...
std::vector<LWECT> FunctionalBootstrapPublicKey::bootstrap(std::vector<std::shared_ptr<FunctionSpecification>> acc_in_vec, const LWECT& lwe_ct_in, const PlaintextEncoding &encoding){   
    // 1) Key switch to \ZZ_Q^{n+1}  
    LWECT lwe_c(ms_from_keyswitch_to_par.from); 
    ...
    // 3) Blind rotate       
    std::shared_ptr<VectorCTAccumulator> acc_one(prepared_acc_builder->get_acc_one(encoding)); 
    std::shared_ptr<BlindRotateOutput> br_out(blind_rotate_output_builder->build()); 
