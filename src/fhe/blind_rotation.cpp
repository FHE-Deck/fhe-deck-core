...
KSFunctionSpecification::KSFunctionSpecification(const std::function<long(long, long)> f, long dim,
    ...
    if(input_encoding.get_plaintext_space() % 2 == 0){
        ...
        for(long i = 0; i < half_plaintext_space; ++i) {    
            poly_msb_0.vec[dim - (int32_t)round((double)(two_dim * i)/(double)input_encoding.get_plaintext_space()) - skip] = -f(i, input_encoding.get_plaintext_space());  
        }   
        for(long i = half_plaintext_space; i < input_encoding.get_plaintext_space(); ++i) {          
            poly_msb_1.vec[two_dim - (int32_t)round((double)(two_dim * i)/(double)input_encoding.get_plaintext_space()) - skip] = f(i, input_encoding.get_plaintext_space()); 
        }   
    }else{ 
        ...
        for(long i = 0; i < half_plaintext_space; ++i) {    
            poly_msb_0.vec[dim - (int32_t)round((double)(two_dim * i)/(double)input_encoding.get_plaintext_space()) - skip] = -f(i, input_encoding.get_plaintext_space()); 
            poly_msb_0.vec[dim - (int32_t)round((double)(two_dim * i)/(double)input_encoding.get_plaintext_space()) - two_skip] = -f(i, input_encoding.get_plaintext_space()); 
        }
           
        poly_msb_0.vec[0] = -f(half_plaintext_space, input_encoding.get_plaintext_space());  
        poly_msb_1.vec[dim - skip] = f(half_plaintext_space, input_encoding.get_plaintext_space()); 

        for(long i = half_plaintext_space+1; i < input_encoding.get_plaintext_space(); ++i) {        
            poly_msb_1.vec[two_dim - (int32_t)round((double)(two_dim * i)/(double)input_encoding.get_plaintext_space()) - skip] = f(i, input_encoding.get_plaintext_space());
            poly_msb_1.vec[two_dim - (int32_t)round((double)(two_dim * i)/(double)input_encoding.get_plaintext_space()) - two_skip] = f(i, input_encoding.get_plaintext_space()); 
        }   
    }
...
std::shared_ptr<VectorCTAccumulator> PreparedVectorCTAccumulators::get_acc_one(PlaintextEncoding output_encoding){ 
    ...
    poly.vec[1] = output_encoding.encode_message(1);  
    return builder->prepare_accumulator(poly);
}
