...
test_sum3d_lut3fa(FHEContext& ctx, std::vector<uint64_t> out = { 0 })
{
    ...
    std::cerr << "\rFA0    ";
    Ciphertext gin0 = 0 + 1 * ct_out96 + 2 * ct_out97 + 1 * ct_out0 + 2 * ct_out1;
    std::vector<Ciphertext> gout0 = ctx.eval_lut_amortized(&gin0, decomp);

    std::cerr << "\rFA1    ";
    Ciphertext gin1 = 0 + 1 * ct_out144 + 2 * ct_out145 + 1 * ct_out48 + 2 * ct_out49;
    ...
    std::cerr << "\rFA3    ";
    Ciphertext gin3 = 0 + 1 * ct_out120 + 2 * ct_out121 + 1 * ct_out24 + 2 * ct_out25;
    std::vector<Ciphertext> gout3 = ctx.eval_lut_amortized(&gin3, decomp);

    std::cerr << "\rFA4    ";
    Ciphertext gin4 = 0 + 1 * ct_out136 + 2 * ct_out137 + 1 * ct_out40 + 2 * ct_out41;
    std::vector<Ciphertext> gout4 = ctx.eval_lut_amortized(&gin4, decomp);

    std::cerr << "\rFA5    ";
    Ciphertext gin5 = gout2[2] + 1 * ct_out170 + 2 * ct_out171 + 1 * ct_out74 + 2 * ct_out75;
    std::vector<Ciphertext> gout5 = ctx.eval_lut_amortized(&gin5, decomp);

    std::cerr << "\rFA6    ";
    Ciphertext gin6 = gout3[2] + 1 * ct_out122 + 2 * ct_out123 + 1 * ct_out26 + 2 * ct_out27;
    std::vector<Ciphertext> gout6 = ctx.eval_lut_amortized(&gin6, decomp);

    std::cerr << "\rFA7    ";
    Ciphertext gin7 = 0 + 1 * ct_out128 + 2 * ct_out129 + 1 * ct_out32 + 2 * ct_out33;
    std::vector<Ciphertext> gout7 = ctx.eval_lut_amortized(&gin7, decomp);

    std::cerr << "\rFA8    ";
    Ciphertext gin8 = gout0[2] + 1 * ct_out98 + 2 * ct_out99 + 1 * ct_out2 + 2 * ct_out3;
    std::vector<Ciphertext> gout8 = ctx.eval_lut_amortized(&gin8, decomp);

    std::cerr << "\rFA9    ";
    Ciphertext gin9 = 0 + 1 * ct_out152 + 2 * ct_out153 + 1 * ct_out56 + 2 * ct_out57;
    std::vector<Ciphertext> gout9 = ctx.eval_lut_amortized(&gin9, decomp);

    std::cerr << "\rFA10    ";
    Ciphertext gin10 = gout8[2] + 1 * ct_out100 + 2 * ct_out101 + 1 * ct_out4 + 2 * ct_out5;
    std::vector<Ciphertext> gout10 = ctx.eval_lut_amortized(&gin10, decomp);

    std::cerr << "\rFA11    ";
    Ciphertext gin11 = 0 + 1 * ct_out176 + 2 * ct_out177 + 1 * ct_out80 + 2 * ct_out81;
    std::vector<Ciphertext> gout11 = ctx.eval_lut_amortized(&gin11, decomp);

    std::cerr << "\rFA12    ";
    Ciphertext gin12 = 0 + 1 * ct_out112 + 2 * ct_out113 + 1 * ct_out16 + 2 * ct_out17;
    std::vector<Ciphertext> gout12 = ctx.eval_lut_amortized(&gin12, decomp);

    std::cerr << "\rFA13    ";
    Ciphertext gin13 = gout4[2] + 1 * ct_out138 + 2 * ct_out139 + 1 * ct_out42 + 2 * ct_out43;
    std::vector<Ciphertext> gout13 = ctx.eval_lut_amortized(&gin13, decomp);

    std::cerr << "\rFA14    ";
    Ciphertext gin14 = gout5[2] + 1 * ct_out172 + 2 * ct_out173 + 1 * ct_out76 + 2 * ct_out77;
    std::vector<Ciphertext> gout14 = ctx.eval_lut_amortized(&gin14, decomp);

    std::cerr << "\rFA15    ";
    Ciphertext gin15 = gout6[2] + 1 * ct_out124 + 2 * ct_out125 + 1 * ct_out28 + 2 * ct_out29;
    std::vector<Ciphertext> gout15 = ctx.eval_lut_amortized(&gin15, decomp);

    std::cerr << "\rFA16    ";
    Ciphertext gin16 = 0 + 1 * ct_out184 + 2 * ct_out185 + 1 * ct_out88 + 2 * ct_out89;
    std::vector<Ciphertext> gout16 = ctx.eval_lut_amortized(&gin16, decomp);

    std::cerr << "\rFA17    ";
    Ciphertext gin17 = 0 + 1 * gout7[0] + 2 * gout7[1] + 1 * gout4[0] + 2 * gout4[1];
    std::vector<Ciphertext> gout17 = ctx.eval_lut_amortized(&gin17, decomp);

    std::cerr << "\rFA18    ";
    Ciphertext gin18 = gout7[2] + 1 * ct_out130 + 2 * ct_out131 + 1 * ct_out34 + 2 * ct_out35;
    std::vector<Ciphertext> gout18 = ctx.eval_lut_amortized(&gin18, decomp);

    std::cerr << "\rFA19    ";
    Ciphertext gin19 = 0 + 1 * gout1[0] + 2 * gout1[1] + 1 * gout9[0] + 2 * gout9[1];
    ...
    std::cerr << "\rFA21    ";
    Ciphertext gin21 = gout10[2] + 1 * ct_out102 + 2 * ct_out103 + 1 * ct_out6 + 2 * ct_out7;
    std::vector<Ciphertext> gout21 = ctx.eval_lut_amortized(&gin21, decomp);

    std::cerr << "\rFA22    ";
    Ciphertext gin22 = 0 + 1 * ct_out104 + 2 * ct_out105 + 1 * ct_out8 + 2 * ct_out9;
    std::vector<Ciphertext> gout22 = ctx.eval_lut_amortized(&gin22, decomp);

    std::cerr << "\rFA23    ";
    Ciphertext gin23 = gout1[2] + 1 * ct_out146 + 2 * ct_out147 + 1 * ct_out50 + 2 * ct_out51;
    std::vector<Ciphertext> gout23 = ctx.eval_lut_amortized(&gin23, decomp);

    std::cerr << "\rFA24    ";
    Ciphertext gin24 = gout9[2] + 1 * ct_out154 + 2 * ct_out155 + 1 * ct_out58 + 2 * ct_out59;
    std::vector<Ciphertext> gout24 = ctx.eval_lut_amortized(&gin24, decomp);

    std::cerr << "\rFA25    ";
    Ciphertext gin25 = gout14[2] + 1 * ct_out174 + 2 * ct_out175 + 1 * ct_out78 + 2 * ct_out79;
    std::vector<Ciphertext> gout25 = ctx.eval_lut_amortized(&gin25, decomp);

    std::cerr << "\rFA26    ";
    Ciphertext gin26 = gout15[2] + 1 * ct_out126 + 2 * ct_out127 + 1 * ct_out30 + 2 * ct_out31;
    std::vector<Ciphertext> gout26 = ctx.eval_lut_amortized(&gin26, decomp);

    std::cerr << "\rFA27    ";
    Ciphertext gin27 = 0 + 1 * gout17[0] + 2 * gout17[1] + 1 * gout19[0] + 2 * gout19[1];
    std::vector<Ciphertext> gout27 = ctx.eval_lut_amortized(&gin27, decomp);

    std::cerr << "\rFA28    ";
    Ciphertext gin28 = gout12[2] + 1 * ct_out114 + 2 * ct_out115 + 1 * ct_out18 + 2 * ct_out19;
    std::vector<Ciphertext> gout28 = ctx.eval_lut_amortized(&gin28, decomp);

    std::cerr << "\rFA29    ";
    Ciphertext gin29 = gout13[2] + 1 * ct_out140 + 2 * ct_out141 + 1 * ct_out44 + 2 * ct_out45;
    std::vector<Ciphertext> gout29 = ctx.eval_lut_amortized(&gin29, decomp);

    std::cerr << "\rFA30    ";
    Ciphertext gin30 = 0 + 1 * gout11[0] + 2 * gout11[1] + 1 * gout16[0] + 2 * gout16[1];
    std::vector<Ciphertext> gout30 = ctx.eval_lut_amortized(&gin30, decomp);

    std::cerr << "\rFA31    ";
    Ciphertext gin31 = gout23[2] + 1 * ct_out148 + 2 * ct_out149 + 1 * ct_out52 + 2 * ct_out53;
    std::vector<Ciphertext> gout31 = ctx.eval_lut_amortized(&gin31, decomp);

    std::cerr << "\rFA32    ";
    Ciphertext gin32 = gout11[2] + 1 * ct_out178 + 2 * ct_out179 + 1 * ct_out82 + 2 * ct_out83;
    std::vector<Ciphertext> gout32 = ctx.eval_lut_amortized(&gin32, decomp);

    std::cerr << "\rFA33    ";
    Ciphertext gin33 = gout18[2] + 1 * ct_out132 + 2 * ct_out133 + 1 * ct_out36 + 2 * ct_out37;
    std::vector<Ciphertext> gout33 = ctx.eval_lut_amortized(&gin33, decomp);

    std::cerr << "\rFA34    ";
    Ciphertext gin34 = 0 + 1 * gout12[0] + 2 * gout12[1] + 1 * gout3[0] + 2 * gout3[1];
    std::vector<Ciphertext> gout34 = ctx.eval_lut_amortized(&gin34, decomp);

    std::cerr << "\rFA35    ";
    Ciphertext gin35 = gout24[2] + 1 * ct_out156 + 2 * ct_out157 + 1 * ct_out60 + 2 * ct_out61;
    std::vector<Ciphertext> gout35 = ctx.eval_lut_amortized(&gin35, decomp);

    std::cerr << "\rFA36    ";
    Ciphertext gin36 = gout19[2] + 1 * gout23[0] + 2 * gout23[1] + 1 * gout24[0] + 2 * gout24[1];
    std::vector<Ciphertext> gout36 = ctx.eval_lut_amortized(&gin36, decomp);

    std::cerr << "\rFA37    ";
    Ciphertext gin37 = gout17[2] + 1 * gout18[0] + 2 * gout18[1] + 1 * gout13[0] + 2 * gout13[1];
    std::vector<Ciphertext> gout37 = ctx.eval_lut_amortized(&gin37, decomp);

    std::cerr << "\rFA38    ";
    Ciphertext gin38 = gout34[2] + 1 * gout28[0] + 2 * gout28[1] + 1 * gout6[0] + 2 * gout6[1];
    std::vector<Ciphertext> gout38 = ctx.eval_lut_amortized(&gin38, decomp);

    std::cerr << "\rFA39    ";
    Ciphertext gin39 = gout20[2] + 1 * ct_out162 + 2 * ct_out163 + 1 * ct_out66 + 2 * ct_out67;
    std::vector<Ciphertext> gout39 = ctx.eval_lut_amortized(&gin39, decomp);

    std::cerr << "\rFA40    ";
    Ciphertext gin40 = 0 + 1 * gout0[0] + 2 * gout0[1] + 1 * gout22[0] + 2 * gout22[1];
    std::vector<Ciphertext> gout40 = ctx.eval_lut_amortized(&gin40, decomp);

    std::cerr << "\rFA41    ";
    Ciphertext gin41 = gout28[2] + 1 * ct_out116 + 2 * ct_out117 + 1 * ct_out20 + 2 * ct_out21;
    std::vector<Ciphertext> gout41 = ctx.eval_lut_amortized(&gin41, decomp);

    std::cerr << "\rFA42    ";
    Ciphertext gin42 = gout29[2] + 1 * ct_out142 + 2 * ct_out143 + 1 * ct_out46 + 2 * ct_out47;
    std::vector<Ciphertext> gout42 = ctx.eval_lut_amortized(&gin42, decomp);

    std::cerr << "\rFA43    ";
    Ciphertext gin43 = gout33[2] + 1 * ct_out134 + 2 * ct_out135 + 1 * ct_out38 + 2 * ct_out39;
    std::vector<Ciphertext> gout43 = ctx.eval_lut_amortized(&gin43, decomp);

    std::cerr << "\rFA44    ";
    Ciphertext gin44 = gout27[2] + 1 * gout37[0] + 2 * gout37[1] + 1 * gout36[0] + 2 * gout36[1];
    std::vector<Ciphertext> gout44 = ctx.eval_lut_amortized(&gin44, decomp);

    std::cerr << "\rFA45    ";
    Ciphertext gin45 = gout38[2] + 1 * gout41[0] + 2 * gout41[1] + 1 * gout15[0] + 2 * gout15[1];
    std::vector<Ciphertext> gout45 = ctx.eval_lut_amortized(&gin45, decomp);

    std::cerr << "\rFA46    ";
    Ciphertext gin46 = gout16[2] + 1 * ct_out186 + 2 * ct_out187 + 1 * ct_out90 + 2 * ct_out91;
    std::vector<Ciphertext> gout46 = ctx.eval_lut_amortized(&gin46, decomp);

    std::cerr << "\rFA47    ";
    Ciphertext gin47 = gout37[2] + 1 * gout33[0] + 2 * gout33[1] + 1 * gout29[0] + 2 * gout29[1];
    std::vector<Ciphertext> gout47 = ctx.eval_lut_amortized(&gin47, decomp);

    std::cerr << "\rFA48    ";
    Ciphertext gin48 = 0 + 1 * gout20[0] + 2 * gout20[1] + 1 * gout2[0] + 2 * gout2[1];
    std::vector<Ciphertext> gout48 = ctx.eval_lut_amortized(&gin48, decomp);

    std::cerr << "\rFA49    ";
    Ciphertext gin49 = gout30[2] + 1 * gout32[0] + 2 * gout32[1] + 1 * gout46[0] + 2 * gout46[1];
    std::vector<Ciphertext> gout49 = ctx.eval_lut_amortized(&gin49, decomp);

    std::cerr << "\rFA50    ";
    Ciphertext gin50 = gout22[2] + 1 * ct_out106 + 2 * ct_out107 + 1 * ct_out10 + 2 * ct_out11;
    ...
    std::cerr << "\rFA52    ";
    Ciphertext gin52 = gout32[2] + 1 * ct_out180 + 2 * ct_out181 + 1 * ct_out84 + 2 * ct_out85;
    std::vector<Ciphertext> gout52 = ctx.eval_lut_amortized(&gin52, decomp);

    std::cerr << "\rFA53    ";
    Ciphertext gin53 = gout41[2] + 1 * ct_out118 + 2 * ct_out119 + 1 * ct_out22 + 2 * ct_out23;
    std::vector<Ciphertext> gout53 = ctx.eval_lut_amortized(&gin53, decomp);

    std::cerr << "\rFA54    ";
    Ciphertext gin54 = 0 + 1 * gout40[0] + 2 * gout40[1] + 1 * gout34[0] + 2 * gout34[1];
    std::vector<Ciphertext> gout54 = ctx.eval_lut_amortized(&gin54, decomp);

    std::cerr << "\rFA55    ";
    Ciphertext gin55 = gout39[2] + 1 * ct_out164 + 2 * ct_out165 + 1 * ct_out68 + 2 * ct_out69;
    std::vector<Ciphertext> gout55 = ctx.eval_lut_amortized(&gin55, decomp);

    std::cerr << "\rFA56    ";
    Ciphertext gin56 = gout35[2] + 1 * ct_out158 + 2 * ct_out159 + 1 * ct_out62 + 2 * ct_out63;
    std::vector<Ciphertext> gout56 = ctx.eval_lut_amortized(&gin56, decomp);

    std::cerr << "\rFA57    ";
    Ciphertext gin57 = gout36[2] + 1 * gout31[0] + 2 * gout31[1] + 1 * gout35[0] + 2 * gout35[1];
    std::vector<Ciphertext> gout57 = ctx.eval_lut_amortized(&gin57, decomp);

    std::cerr << "\rFA58    ";
    Ciphertext gin58 = gout52[2] + 1 * ct_out182 + 2 * ct_out183 + 1 * ct_out86 + 2 * ct_out87;
    std::vector<Ciphertext> gout58 = ctx.eval_lut_amortized(&gin58, decomp);

    std::cerr << "\rFA59    ";
    Ciphertext gin59 = gout40[2] + 1 * gout8[0] + 2 * gout8[1] + 1 * gout50[0] + 2 * gout50[1];
    std::vector<Ciphertext> gout59 = ctx.eval_lut_amortized(&gin59, decomp);

    std::cerr << "\rFA60    ";
    Ciphertext gin60 = gout44[2] + 1 * gout47[0] + 2 * gout47[1] + 1 * gout57[0] + 2 * gout57[1];
    std::vector<Ciphertext> gout60 = ctx.eval_lut_amortized(&gin60, decomp);

    std::cerr << "\rFA61    ";
    Ciphertext gin61 = gout50[2] + 1 * ct_out108 + 2 * ct_out109 + 1 * ct_out12 + 2 * ct_out13;
    std::vector<Ciphertext> gout61 = ctx.eval_lut_amortized(&gin61, decomp);

    std::cerr << "\rFA62    ";
    Ciphertext gin62 = gout48[2] + 1 * gout39[0] + 2 * gout39[1] + 1 * gout5[0] + 2 * gout5[1];
    std::vector<Ciphertext> gout62 = ctx.eval_lut_amortized(&gin62, decomp);

    std::cerr << "\rFA63    ";
    Ciphertext gin63 = 0 + 1 * gout54[0] + 2 * gout54[1] + 1 * gout27[0] + 2 * gout27[1];
    std::vector<Ciphertext> gout63 = ctx.eval_lut_amortized(&gin63, decomp);

    std::cerr << "\rFA64    ";
    Ciphertext gin64 = gout46[2] + 1 * ct_out188 + 2 * ct_out189 + 1 * ct_out92 + 2 * ct_out93;
    std::vector<Ciphertext> gout64 = ctx.eval_lut_amortized(&gin64, decomp);

    std::cerr << "\rFA65    ";
    Ciphertext gin65 = gout55[2] + 1 * ct_out166 + 2 * ct_out167 + 1 * ct_out70 + 2 * ct_out71;
    std::vector<Ciphertext> gout65 = ctx.eval_lut_amortized(&gin65, decomp);

    std::cerr << "\rFA66    ";
    Ciphertext gin66 = gout45[2] + 1 * gout53[0] + 2 * gout53[1] + 1 * gout26[0] + 2 * gout26[1];
    std::vector<Ciphertext> gout66 = ctx.eval_lut_amortized(&gin66, decomp);

    std::cerr << "\rFA67    ";
    Ciphertext gin67 = gout57[2] + 1 * gout51[0] + 2 * gout51[1] + 1 * gout56[0] + 2 * gout56[1];
    std::vector<Ciphertext> gout67 = ctx.eval_lut_amortized(&gin67, decomp);

    std::cerr << "\rFA68    ";
    Ciphertext gin68 = gout61[2] + 1 * ct_out110 + 2 * ct_out111 + 1 * ct_out14 + 2 * ct_out15;
    std::vector<Ciphertext> gout68 = ctx.eval_lut_amortized(&gin68, decomp);

    std::cerr << "\rFA69    ";
    Ciphertext gin69 = gout47[2] + 1 * gout43[0] + 2 * gout43[1] + 1 * gout42[0] + 2 * gout42[1];
    std::vector<Ciphertext> gout69 = ctx.eval_lut_amortized(&gin69, decomp);

    std::cerr << "\rFA70    ";
    Ciphertext gin70 = gout66[2] + 1 * gout53[2] + 1 * gout26[2];
    std::vector<Ciphertext> gout70 = ctx.eval_lut_amortized(&gin70, decomp);

    std::cerr << "\rFA71    ";
    Ciphertext gin71 = 0 + 1 * gout48[0] + 2 * gout48[1] + 1 * gout30[0] + 2 * gout30[1];
    std::vector<Ciphertext> gout71 = ctx.eval_lut_amortized(&gin71, decomp);

    std::cerr << "\rFA72    ";
    Ciphertext gin72 = gout60[2] + 1 * gout69[0] + 2 * gout69[1] + 1 * gout67[0] + 2 * gout67[1];
    std::vector<Ciphertext> gout72 = ctx.eval_lut_amortized(&gin72, decomp);

    std::cerr << "\rFA73    ";
    Ciphertext gin73 = gout59[2] + 1 * gout10[0] + 2 * gout10[1] + 1 * gout61[0] + 2 * gout61[1];
    std::vector<Ciphertext> gout73 = ctx.eval_lut_amortized(&gin73, decomp);

    std::cerr << "\rFA74    ";
    Ciphertext gin74 = gout64[2] + 1 * ct_out190 + 2 * ct_out191 + 1 * ct_out94 + 2 * ct_out95;
    std::vector<Ciphertext> gout74 = ctx.eval_lut_amortized(&gin74, decomp);

    std::cerr << "\rFA75    ";
    Ciphertext gin75 = gout54[2] + 1 * gout59[0] + 2 * gout59[1] + 1 * gout38[0] + 2 * gout38[1];
    std::vector<Ciphertext> gout75 = ctx.eval_lut_amortized(&gin75, decomp);

    std::cerr << "\rFA76    ";
    Ciphertext gin76 = gout69[2] + 1 * gout43[2] + 1 * gout42[2];
    std::vector<Ciphertext> gout76 = ctx.eval_lut_amortized(&gin76, decomp);

    std::cerr << "\rFA77    ";
    Ciphertext gin77 = gout49[2] + 1 * gout52[0] + 2 * gout52[1] + 1 * gout64[0] + 2 * gout64[1];
    std::vector<Ciphertext> gout77 = ctx.eval_lut_amortized(&gin77, decomp);

    std::cerr << "\rFA78    ";
    Ciphertext gin78 = gout62[2] + 1 * gout55[0] + 2 * gout55[1] + 1 * gout14[0] + 2 * gout14[1];
    std::vector<Ciphertext> gout78 = ctx.eval_lut_amortized(&gin78, decomp);

    std::cerr << "\rFA79    ";
    Ciphertext gin79 = 0 + 1 * gout63[0] + 2 * gout63[1] + 1 * gout71[0] + 2 * gout71[1];
    std::vector<Ciphertext> gout79 = ctx.eval_lut_amortized(&gin79, decomp);

    std::cerr << "\rFA80    ";
    Ciphertext gin80 = gout67[2] + 1 * gout51[2] + 1 * gout56[2];
    std::vector<Ciphertext> gout80 = ctx.eval_lut_amortized(&gin80, decomp);

    std::cerr << "\rFA81    ";
    Ciphertext gin81 = gout72[2] + 1 * gout76[0] + 2 * gout76[1] + 1 * gout80[0] + 2 * gout80[1];
    std::vector<Ciphertext> gout81 = ctx.eval_lut_amortized(&gin81, decomp);

    std::cerr << "\rFA82    ";
    Ciphertext gin82 = gout71[2] + 1 * gout62[0] + 2 * gout62[1] + 1 * gout49[0] + 2 * gout49[1];
    std::vector<Ciphertext> gout82 = ctx.eval_lut_amortized(&gin82, decomp);

    std::cerr << "\rFA83    ";
    Ciphertext gin83 = gout75[2] + 1 * gout73[0] + 2 * gout73[1] + 1 * gout45[0] + 2 * gout45[1];
    std::vector<Ciphertext> gout83 = ctx.eval_lut_amortized(&gin83, decomp);

    std::cerr << "\rFA84    ";
    Ciphertext gin84 = gout63[2] + 1 * gout75[0] + 2 * gout75[1] + 1 * gout44[0] + 2 * gout44[1];
    std::vector<Ciphertext> gout84 = ctx.eval_lut_amortized(&gin84, decomp);

    std::cerr << "\rFA85    ";
    Ciphertext gin85 = gout73[2] + 1 * gout21[0] + 2 * gout21[1] + 1 * gout68[0] + 2 * gout68[1];
    std::vector<Ciphertext> gout85 = ctx.eval_lut_amortized(&gin85, decomp);

    std::cerr << "\rFA86    ";
    Ciphertext gin86 = gout79[2] + 1 * gout84[0] + 2 * gout84[1] + 1 * gout82[0] + 2 * gout82[1];
    std::vector<Ciphertext> gout86 = ctx.eval_lut_amortized(&gin86, decomp);

    std::cerr << "\rFA87    ";
    Ciphertext gin87 = gout77[2] + 1 * gout58[0] + 2 * gout58[1] + 1 * gout74[0] + 2 * gout74[1];
    std::vector<Ciphertext> gout87 = ctx.eval_lut_amortized(&gin87, decomp);

    std::cerr << "\rFA88    ";
    Ciphertext gin88 = gout78[2] + 1 * gout65[0] + 2 * gout65[1] + 1 * gout25[0] + 2 * gout25[1];
    std::vector<Ciphertext> gout88 = ctx.eval_lut_amortized(&gin88, decomp);

    std::cerr << "\rFA89    ";
    Ciphertext gin89 = gout82[2] + 1 * gout78[0] + 2 * gout78[1] + 1 * gout77[0] + 2 * gout77[1];
    std::vector<Ciphertext> gout89 = ctx.eval_lut_amortized(&gin89, decomp);

    std::cerr << "\rFA90    ";
    Ciphertext gin90 = gout83[2] + 1 * gout85[0] + 2 * gout85[1] + 1 * gout66[0] + 2 * gout66[1];
    std::vector<Ciphertext> gout90 = ctx.eval_lut_amortized(&gin90, decomp);

    std::cerr << "\rFA91    ";
    Ciphertext gin91 = gout84[2] + 1 * gout83[0] + 2 * gout83[1] + 1 * gout60[0] + 2 * gout60[1];
    std::vector<Ciphertext> gout91 = ctx.eval_lut_amortized(&gin91, decomp);

    std::cerr << "\rFA92    ";
    Ciphertext gin92 = gout85[2] + 1 * gout21[2] + 1 * gout68[2];
    std::vector<Ciphertext> gout92 = ctx.eval_lut_amortized(&gin92, decomp);

    std::cerr << "\rFA93    ";
    Ciphertext gin93 = gout87[2] + 1 * gout58[2] + 1 * gout74[2];
    std::vector<Ciphertext> gout93 = ctx.eval_lut_amortized(&gin93, decomp);

    std::cerr << "\rFA94    ";
    Ciphertext gin94 = gout91[2] + 1 * gout90[0] + 2 * gout90[1] + 1 * gout72[0] + 2 * gout72[1];
    std::vector<Ciphertext> gout94 = ctx.eval_lut_amortized(&gin94, decomp);

    std::cerr << "\rFA95    ";
    Ciphertext gin95 = gout89[2] + 1 * gout88[0] + 2 * gout88[1] + 1 * gout87[0] + 2 * gout87[1];
    std::vector<Ciphertext> gout95 = ctx.eval_lut_amortized(&gin95, decomp);

    std::cerr << "\rFA96    ";
    Ciphertext gin96 = gout86[2] + 1 * gout91[0] + 2 * gout91[1] + 1 * gout89[0] + 2 * gout89[1];
    std::vector<Ciphertext> gout96 = ctx.eval_lut_amortized(&gin96, decomp);

    std::cerr << "\rFA97    ";
    Ciphertext gin97 = gout90[2] + 1 * gout92[0] + 2 * gout92[1] + 1 * gout70[0] + 2 * gout70[1];
    ...
    std::cerr << "\rFA99    ";
    Ciphertext gin99 = gout88[2] + 1 * gout65[2] + 1 * gout25[2];
    std::vector<Ciphertext> gout99 = ctx.eval_lut_amortized(&gin99, decomp);

    std::cerr << "\rFA100    ";
    Ciphertext gin100 = gout95[2] + 1 * gout99[0] + 2 * gout99[1] + 1 * gout93[0] + 2 * gout93[1];
    std::vector<Ciphertext> gout100 = ctx.eval_lut_amortized(&gin100, decomp);

    std::cerr << "\rFA101    ";
    Ciphertext gin101 = gout94[2] + 1 * gout97[0] + 2 * gout97[1] + 1 * gout81[0] + 2 * gout81[1];
    std::vector<Ciphertext> gout101 = ctx.eval_lut_amortized(&gin101, decomp);

    std::cerr << "\rFA102    ";
    Ciphertext gin102 = gout98[2] + 1 * gout101[0] + 2 * gout101[1] + 1 * gout100[0] + 2 * gout100[1];
    std::vector<Ciphertext> gout102 = ctx.eval_lut_amortized(&gin102, decomp);

    std::cerr << "\rFA103    ";
    Ciphertext gin103 = gout101[2] + 1 * gout97[2] + 1 * gout81[2];
    std::vector<Ciphertext> gout103 = ctx.eval_lut_amortized(&gin103, decomp);

    std::cerr << "\rFA104    ";
    Ciphertext gin104 = gout102[2] + 1 * gout103[0] + 2 * gout103[1] + 1 * gout100[2] + 2 * 0;
    std::vector<Ciphertext> gout104 = ctx.eval_lut_amortized(&gin104, decomp);

    std::cerr << "\r          \r";
    std::vector<long> test_out;
    test_out.push_back(ctx.decrypt(&gout79[0])); /* out192 */
    test_out.push_back(ctx.decrypt(&gout79[1])); /* out193 */
    test_out.push_back(ctx.decrypt(&gout86[0])); /* out194 */
    test_out.push_back(ctx.decrypt(&gout86[1])); /* out195 */
    test_out.push_back(ctx.decrypt(&gout96[0])); /* out196 */
    test_out.push_back(ctx.decrypt(&gout96[1])); /* out197 */
    test_out.push_back(ctx.decrypt(&gout98[0])); /* out198 */
    test_out.push_back(ctx.decrypt(&gout98[1])); /* out199 */
    test_out.push_back(ctx.decrypt(&gout102[0])); /* out200 */
    test_out.push_back(ctx.decrypt(&gout102[1])); /* out201 */
