...
test_sqrt_boolean(FHEContext& ctx, uint16_t num = 0)
{
    ...
    std::cerr << "\rLUT54   ";
    Ciphertext gin54 = 2 * gout21 + gout50;
    Ciphertext gout54 = ctx.eval_lut(&gin54, nand2);
    std::cerr << "\rLUT55   ";
    Ciphertext gin55 = 2 * gout49 + gout52;
    Ciphertext gout55 = ctx.eval_lut(&gin55, nand2);
    std::cerr << "\rLUT56   ";
    Ciphertext gin56 = 2 * (1 - gout50) + gout41;
    Ciphertext gout56 = ctx.eval_lut(&gin56, and2);
    std::cerr << "\rLUT57   ";
    Ciphertext gin57 = 2 * gout54 + gout41;
    ...
    std::cerr << "\rLUT61   ";
    Ciphertext gin61 = 2 * gout11 + gout60;
    Ciphertext gout61 = ctx.eval_lut(&gin61, nand2);
    std::cerr << "\rLUT62   ";
    Ciphertext gin62 = 2 * gout4 + gout61;
    ...
    Ciphertext gin66B = 2 * gout15 + (1 - ct_num8);
    ...
    std::cerr << "\rLUT80   ";
    Ciphertext gin80 = 2 * gout38 + gout76;
    Ciphertext gout80 = ctx.eval_lut(&gin80, nand2);
    std::cerr << "\rLUT81   ";
    Ciphertext gin81 = 2 * gout42 + gout77;
    ...
    std::cerr << "\rLUT92   ";
    Ciphertext gin92 = 2 * gout11 + gout90;
    ...
    std::cerr << "\rLUT94   ";
    Ciphertext gin94 = 2 * (1 - gout92) + gout18;
    Ciphertext gout94 = ctx.eval_lut(&gin94, and2);
    std::cerr << "\rLUT95   ";
    Ciphertext gin95 = 2 * gout18 + gout92;
    Ciphertext gout95 = ctx.eval_lut(&gin95, xnor2);
    std::cerr << "\rIMUX96  ";
    Ciphertext gin96A = 2 * gout76 + gout92;
    Ciphertext gout96A = ctx.eval_lut(&gin96A, and2);
    Ciphertext gin96B = 2 * gout83 + (1 - gout92);
    Ciphertext gout96B = ctx.eval_lut(&gin96B, and2);
    Ciphertext gout96 = gout96A + gout96B;
    std::cerr << "\rLUT97   ";
    Ciphertext gin97 = 2 * gout15 + gout92;
    Ciphertext gout97 = ctx.eval_lut(&gin97, or2);
    std::cerr << "\rIMUX98  ";
    Ciphertext gin98A = 2 * gout71 + gout92;
    Ciphertext gout98A = ctx.eval_lut(&gin98A, and2);
    Ciphertext gin98B = 2 * gout85 + (1 - gout92);
    Ciphertext gout98B = ctx.eval_lut(&gin98B, and2);
    Ciphertext gout98 = gout98A + gout98B;
    std::cerr << "\rINV99   ";
    Ciphertext gout99 = 1 - gout92;

    std::cerr << "\rLUT100   ";
    Ciphertext gin100 = 2 * ct_num7 + gout94;
    Ciphertext gout100 = ctx.eval_lut(&gin100, xnor2);
    std::cerr << "\rLUT101   ";
    Ciphertext gin101 = 2 * gout33 + gout96;
    Ciphertext gout101 = ctx.eval_lut(&gin101, or2);
    std::cerr << "\rLUT102   ";
    Ciphertext gin102 = 2 * gout97 + gout91;
    Ciphertext gout102 = ctx.eval_lut(&gin102, and2);
    std::cerr << "\rLUT103   ";
    Ciphertext gin103 = 2 * gout29 + gout98;
    Ciphertext gout103 = ctx.eval_lut(&gin103, xnor2);
    std::cerr << "\rLUT104   ";
    Ciphertext gin104 = 2 * gout33 + gout96;
    Ciphertext gout104 = ctx.eval_lut(&gin104, xnor2);
    std::cerr << "\rLUT105   ";
    Ciphertext gin105 = 2 * gout21 + gout98;
    Ciphertext gout105 = ctx.eval_lut(&gin105, nand2);
    std::cerr << "\rINV106   ";
    Ciphertext gout106 = 1 - gout98;

    std::cerr << "\rIMUX107  ";
    Ciphertext gin107A = 2 * gout99 + ct_num6;
    Ciphertext gout107A = ctx.eval_lut(&gin107A, and2);
    Ciphertext gin107B = 2 * gout5 + (1 - ct_num6);
    ...
    std::cerr << "\rINV110   ";
    Ciphertext gout110 = 1 - gout100;

    std::cerr << "\rLUT111   ";
    Ciphertext gin111 = 2 * (1 - gout93) + gout105;
    Ciphertext gout111 = ctx.eval_lut(&gin111, and2);
    std::cerr << "\rLUT112   ";
    Ciphertext gin112 = 2 * ct_num8 + gout102;
    Ciphertext gout112 = ctx.eval_lut(&gin112, xnor2);
    std::cerr << "\rINV113   ";
    Ciphertext gout113 = 1 - gout107;

    std::cerr << "\rLUT114   ";
    Ciphertext gin114 = 2 * gout109 + gout107;
    Ciphertext gout114 = ctx.eval_lut(&gin114, xnor2);
    std::cerr << "\rINV115   ";
    Ciphertext gout115 = 1 - gout112;

    std::cerr << "\rLUT116   ";
    Ciphertext gin116 = 2 * gout42 + gout112;
    Ciphertext gout116 = ctx.eval_lut(&gin116, nand2);
    std::cerr << "\rLUT117   ";
    Ciphertext gin117 = 2 * gout38 + gout112;
    ...
    std::cerr << "\rLUT140   ";
    Ciphertext gin140 = 2 * gout33 + gout130;
    Ciphertext gout140 = ctx.eval_lut(&gin140, or2);
    std::cerr << "\rLUT141   ";
    Ciphertext gin141 = 2 * gout61 + gout132;
    Ciphertext gout141 = ctx.eval_lut(&gin141, xnor2);
    std::cerr << "\rLUT142   ";
    Ciphertext gin142 = 2 * gout63 + gout132;
    Ciphertext gout142 = ctx.eval_lut(&gin142, or2);
    std::cerr << "\rLUT143   ";
    Ciphertext gin143 = 2 * gout38 + gout137;
    Ciphertext gout143 = ctx.eval_lut(&gin143, xnor2);
    std::cerr << "\rLUT144   ";
    Ciphertext gin144 = 2 * gout14 + gout134;
    Ciphertext gout144 = ctx.eval_lut(&gin144, xnor2);
    std::cerr << "\rLUT145   ";
    Ciphertext gin145 = 2 * gout24 + gout130;
    Ciphertext gout145 = ctx.eval_lut(&gin145, xnor2);
    std::cerr << "\rLUT146   ";
    Ciphertext gin146 = 2 * gout29 + gout139;
    Ciphertext gout146 = ctx.eval_lut(&gin146, or2);
    std::cerr << "\rIMUX147  ";
    Ciphertext gin147A = 2 * gout136 + ct_num4;
    Ciphertext gout147A = ctx.eval_lut(&gin147A, and2);
    Ciphertext gin147B = 2 * gout19 + (1 - ct_num4);
    Ciphertext gout147B = ctx.eval_lut(&gin147B, and2);
    Ciphertext gout147 = gout147A + gout147B;
    std::cerr << "\rLUT148   ";
    Ciphertext gin148 = 2 * gout139 + gout135;
    Ciphertext gout148 = ctx.eval_lut(&gin148, nand2);
    std::cerr << "\rLUT149   ";
    Ciphertext gin149 = 2 * gout21 + gout139;
    Ciphertext gout149 = ctx.eval_lut(&gin149, xnor2);
    std::cerr << "\rLUT150   ";
    Ciphertext gin150 = 2 * gout42 + gout137;
    Ciphertext gout150 = ctx.eval_lut(&gin150, nand2);
    std::cerr << "\rINV151   ";
    Ciphertext gout151 = 1 - gout141;

    std::cerr << "\rLUT152   ";
    Ciphertext gin152 = 2 * gout92 + gout144;
    Ciphertext gout152 = ctx.eval_lut(&gin152, nand2);
    std::cerr << "\rLUT153   ";
    Ciphertext gin153 = 2 * gout99 + gout144;
    Ciphertext gout153 = ctx.eval_lut(&gin153, xnor2);
    std::cerr << "\rINV154   ";
    Ciphertext gout154 = 1 - gout143;

    std::cerr << "\rLUT155   ";
    Ciphertext gin155 = 2 * gout146 + gout135;
    Ciphertext gout155 = ctx.eval_lut(&gin155, and2);
    std::cerr << "\rINV156   ";
    Ciphertext gout156 = 1 - gout147;

    std::cerr << "\rLUT157   ";
    Ciphertext gin157 = 2 * gout153 + gout147;
    Ciphertext gout157 = ctx.eval_lut(&gin157, xnor2);
    std::cerr << "\rLUT158   ";
    Ciphertext gin158 = 2 * gout153 + gout156;
    Ciphertext gout158 = ctx.eval_lut(&gin158, nand2);
    std::cerr << "\rLUT159   ";
    Ciphertext gin159 = 2 * gout152 + gout158;
    Ciphertext gout159 = ctx.eval_lut(&gin159, and2);
    std::cerr << "\rLUT160   ";
    Ciphertext gin160 = 2 * gout151 + gout159;
    Ciphertext gout160 = ctx.eval_lut(&gin160, xnor2);
    std::cerr << "\rLUT161   ";
    Ciphertext gin161 = 2 * gout151 + gout159;
    Ciphertext gout161 = ctx.eval_lut(&gin161, or2);
    std::cerr << "\rLUT162   ";
    Ciphertext gin162 = 2 * gout142 + gout161;
    Ciphertext gout162 = ctx.eval_lut(&gin162, and2);
    std::cerr << "\rLUT163   ";
    Ciphertext gin163 = 2 * gout154 + gout162;
    Ciphertext gout163 = ctx.eval_lut(&gin163, or2);
    std::cerr << "\rLUT164   ";
    Ciphertext gin164 = 2 * gout143 + gout162;
    Ciphertext gout164 = ctx.eval_lut(&gin164, xnor2);
    std::cerr << "\rLUT165   ";
    Ciphertext gin165 = 2 * gout150 + gout163;
    Ciphertext gout165 = ctx.eval_lut(&gin165, nand2);
    std::cerr << "\rLUT166   ";
    Ciphertext gin166 = 2 * gout165 + gout145;
    Ciphertext gout166 = ctx.eval_lut(&gin166, xnor2);
    std::cerr << "\rLUT167   ";
    Ciphertext gin167 = 2 * gout165 + gout145;
    Ciphertext gout167 = ctx.eval_lut(&gin167, nand2);
    std::cerr << "\rLUT168   ";
    Ciphertext gin168 = 2 * gout140 + gout167;
    Ciphertext gout168 = ctx.eval_lut(&gin168, nand2);
    std::cerr << "\rLUT169   ";
    Ciphertext gin169 = 2 * gout149 + gout168;
    Ciphertext gout169 = ctx.eval_lut(&gin169, nand2);
    std::cerr << "\rLUT170   ";
    Ciphertext gin170 = 2 * gout149 + gout168;
    Ciphertext gout170 = ctx.eval_lut(&gin170, xor2);
    std::cerr << "\rLUT171   ";
    Ciphertext gin171 = 2 * gout169 + gout155;
    Ciphertext gout171 = ctx.eval_lut(&gin171, nand2);
    std::cerr << "\rLUT172   ";
    Ciphertext gin172 = 2 * gout11 + gout171;
    ...
    std::cerr << "\rLUT202   ";
    Ciphertext gin202 = 2 * gout201 + gout199;
    ...
    std::cerr << "\rLUT206   ";
    Ciphertext gin206 = 2 * gout11 + gout205;
    Ciphertext gout206 = ctx.eval_lut(&gin206, and2);

    std::cerr << "\r          \r";
    std::vector<long> test_out;
    test_out.push_back(ctx.decrypt(&gout206)); /* out0 */
    test_out.push_back(ctx.decrypt(&gout180)); /* out1 */
    test_out.push_back(ctx.decrypt(&gout136)); /* out2 */
    test_out.push_back(ctx.decrypt(&gout99)); /* out3 */
    test_out.push_back(ctx.decrypt(&gout63)); /* out4 */
    test_out.push_back(ctx.decrypt(&gout38)); /* out5 */
    test_out.push_back(ctx.decrypt(&gout33)); /* out6 */
