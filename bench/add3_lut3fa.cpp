...
std::vector<long>
test_add3_lut3fa(FHEContext& ctx, uint8_t a = 0, uint8_t b = 0)
{
    ...
    std::cerr << "\rFA1    ";
    Ciphertext gin1 = gout0[2] + 1 * ct_a2 + 1 * ct_b2;
...
static void
BM_add3_lut3fa(benchmark::State& state)
{
    FHEContext ctx;
    ...
    for (auto _ : state) {
        test_add3_lut3fa(ctx);
    }
}

BENCHMARK(BM_add3_lut3fa)->Unit(benchmark::kSecond);
BENCHMARK_MAIN();
