...
test_image_ricker_lut4fa(FHEContext& ctx, std::vector<uint64_t> window = { 0 })
{
    ...
    std::cerr << "\rFA0    ";
    Ciphertext gin0 = 0 + 1 * ct_window0 + 2 * ct_window1 + 4 * ct_window2 + 1 * ct_window8 + 2 * ct_window9 + 4 * ct_window10;
    ...
    std::cerr << "\rFA3    ";
    Ciphertext gin3 = 0 + 1 * ct_window40 + 2 * ct_window41 + 4 * ct_window42 + 1 * ct_window48 + 2 * ct_window49 + 4 * ct_window50;
    std::vector<Ciphertext> gout3 = ctx.eval_lut_amortized(&gin3, decomp);

    std::cerr << "\rFA4    ";
    Ciphertext gin4 = gout3[3] + 1 * ct_window43 + 2 * ct_window44 + 4 * ct_window45 + 1 * ct_window51 + 2 * ct_window52 + 4 * ct_window53;
    std::vector<Ciphertext> gout4 = ctx.eval_lut_amortized(&gin4, decomp);

    std::cerr << "\rFA5    ";
    Ciphertext gin5 = 0 + 1 * gout3[0] + 2 * gout3[1] + 4 * gout3[2] + 1 * gout1[0] + 2 * gout1[1] + 4 * gout1[2];
    std::vector<Ciphertext> gout5 = ctx.eval_lut_amortized(&gin5, decomp);

    std::cerr << "\rFA6    ";
    Ciphertext gin6 = gout1[3] + 1 * ct_window59 + 2 * ct_window60 + 4 * ct_window61 + 1 * ct_window67 + 2 * ct_window68 + 4 * ct_window69;
    std::vector<Ciphertext> gout6 = ctx.eval_lut_amortized(&gin6, decomp);

    std::cerr << "\rFA7    ";
    Ciphertext gin7 = gout2[3] + 1 * ct_window19 + 2 * ct_window20 + 4 * ct_window21 + 1 * ct_window27 + 2 * ct_window28 + 4 * ct_window29;
    std::vector<Ciphertext> gout7 = ctx.eval_lut_amortized(&gin7, decomp);

    std::cerr << "\rFA8    ";
    Ciphertext gin8 = 0 + 1 * gout0[0] + 2 * gout0[1] + 4 * gout0[2] + 1 * gout2[0] + 2 * gout2[1] + 4 * gout2[2];
    std::vector<Ciphertext> gout8 = ctx.eval_lut_amortized(&gin8, decomp);

    std::cerr << "\rFA9    ";
    Ciphertext gin9 = gout6[3] + 1 * ct_window62 + 2 * ct_window63 + 1 * ct_window70 + 2 * ct_window71;
    std::vector<Ciphertext> gout9 = ctx.eval_lut_amortized(&gin9, decomp);

    std::cerr << "\rFA10    ";
    Ciphertext gin10 = 0 + 1 * gout8[0] + 2 * gout8[1] + 4 * gout8[2] + 1 * gout5[0] + 2 * gout5[1] + 4 * gout5[2];
    std::vector<Ciphertext> gout10 = ctx.eval_lut_amortized(&gin10, decomp);

    std::cerr << "\rFA11    ";
    Ciphertext gin11 = gout7[3] + 1 * ct_window22 + 2 * ct_window23 + 1 * ct_window30 + 2 * ct_window31;
    std::vector<Ciphertext> gout11 = ctx.eval_lut_amortized(&gin11, decomp);

    std::cerr << "\rFA12    ";
    Ciphertext gin12 = gout0[3] + 1 * ct_window3 + 2 * ct_window4 + 4 * ct_window5 + 1 * ct_window11 + 2 * ct_window12 + 4 * ct_window13;
    ...
    std::cerr << "\rFA14    ";
    Ciphertext gin14 = gout5[3] + 1 * gout4[0] + 2 * gout4[1] + 4 * gout4[2] + 1 * gout6[0] + 2 * gout6[1] + 4 * gout6[2];
    std::vector<Ciphertext> gout14 = ctx.eval_lut_amortized(&gin14, decomp);

    std::cerr << "\rFA15    ";
    Ciphertext gin15 = gout8[3] + 1 * gout12[0] + 2 * gout12[1] + 4 * gout12[2] + 1 * gout7[0] + 2 * gout7[1] + 4 * gout7[2];
    std::vector<Ciphertext> gout15 = ctx.eval_lut_amortized(&gin15, decomp);

    std::cerr << "\rFA16    ";
    Ciphertext gin16 = gout12[3] + 1 * ct_window6 + 2 * ct_window7 + 1 * ct_window14 + 2 * ct_window15;
    std::vector<Ciphertext> gout16 = ctx.eval_lut_amortized(&gin16, decomp);

    std::cerr << "\rFA17    ";
    Ciphertext gin17 = gout14[3] + 1 * gout13[0] + 2 * gout13[1] + 4 * gout13[2] + 1 * gout9[0] + 2 * gout9[1] + 4 * gout9[2];
    std::vector<Ciphertext> gout17 = ctx.eval_lut_amortized(&gin17, decomp);

    std::cerr << "\rFA18    ";
    Ciphertext gin18 = gout15[3] + 1 * gout16[0] + 2 * gout16[1] + 4 * gout16[2] + 1 * gout11[0] + 2 * gout11[1] + 4 * gout11[2];
    std::vector<Ciphertext> gout18 = ctx.eval_lut_amortized(&gin18, decomp);

    std::cerr << "\rFA19    ";
    Ciphertext gin19 = gout10[3] + 1 * gout15[0] + 2 * gout15[1] + 4 * gout15[2] + 1 * gout14[0] + 2 * gout14[1] + 4 * gout14[2];
    ...
    gout21.push_back(1 - gout19[1]);

    std::cerr << "\rINV22   ";
    std::vector<Ciphertext> gout22;
    gout22.push_back(1 - gout19[2]);

    std::cerr << "\rINV23   ";
    std::vector<Ciphertext> gout23;
    gout23.push_back(1 - gout20[0]);

    std::cerr << "\rINV24   ";
    std::vector<Ciphertext> gout24;
    gout24.push_back(1 - gout20[1]);

    std::cerr << "\rINV25   ";
    std::vector<Ciphertext> gout25;
    gout25.push_back(1 - gout20[2]);

    std::cerr << "\rFA26    ";
    Ciphertext gin26 = gout20[3] + 1 * gout18[3] + 1 * gout17[3];
    std::vector<Ciphertext> gout26 = ctx.eval_lut_amortized(&gin26, decomp);

    std::cerr << "\rINV27   ";
    std::vector<Ciphertext> gout27;
    gout27.push_back(1 - gout19[0]);

    std::cerr << "\rFA28    ";
    Ciphertext gin28 = 1 + 1 * ct_window32 + 2 * ct_window33 + 4 * ct_window34 + 1 * gout27[0] + 2 * gout21[0] + 4 * gout22[0];
    std::vector<Ciphertext> gout28 = ctx.eval_lut_amortized(&gin28, decomp);

    std::cerr << "\rINV29   ";
    std::vector<Ciphertext> gout29;
    gout29.push_back(1 - gout26[0]);

    std::cerr << "\rINV30   ";
    std::vector<Ciphertext> gout30;
    gout30.push_back(1 - gout26[1]);

    std::cerr << "\rFA31    ";
    Ciphertext gin31 = gout28[3] + 1 * ct_window35 + 2 * ct_window36 + 4 * ct_window37 + 1 * gout23[0] + 2 * gout24[0] + 4 * gout25[0];
    ...
    std::cerr << "\rLUT32   ";
    Ciphertext gin32 = 1 * gout28[2] + 2 * gout28[0] + 4 * gout28[1];
    std::vector<Ciphertext> gout32 = ctx.eval_lut_amortized(&gin32, lut32);

    std::cerr << "\rFA33    ";
    Ciphertext gin33 = gout31[3] + 1 * ct_window38 + 2 * ct_window39 + 1 * gout29[0] + 2 * gout30[0];
    ...
    std::cerr << "\rLUT34   ";
    Ciphertext gin34 = 1 * gout28[2] + 2 * gout28[0] + 4 * gout28[1] + 8 * gout33[2];
    ...
    std::cerr << "\rLUT35   ";
    Ciphertext gin35 = 1 * gout28[2] + 2 * gout31[0] + 4 * gout28[0] + 8 * gout28[1];
    ...
    std::cerr << "\rLUT36   ";
    Ciphertext gin36 = 1 * gout35[0] + 2 * gout31[1];
    ...
    std::cerr << "\rLUT37   ";
    Ciphertext gin37 = 1 * gout32[0] + 2 * gout33[2];
    ...
    std::cerr << "\rLUT38   ";
    Ciphertext gin38 = 1 * gout31[2] + 2 * gout36[0] + 4 * gout33[0] + 8 * gout33[2];
    ...
    std::cerr << "\rLUT39   ";
    Ciphertext gin39 = 1 * gout35[0] + 2 * gout33[2];
    ...
    std::cerr << "\rLUT40   ";
    Ciphertext gin40 = 1 * gout35[0] + 2 * gout31[1] + 4 * gout31[2] + 8 * gout33[0];
    ...
    std::cerr << "\rLUT41   ";
    Ciphertext gin41 = 1 * gout37[0] + 2 * gout31[0];
    ...
    std::cerr << "\rLUT42   ";
    Ciphertext gin42 = 1 * gout38[1] + 2 * gout31[2];
    ...
    std::cerr << "\rLUT43   ";
    Ciphertext gin43 = 1 * gout39[0] + 2 * gout31[1];
    ...
    std::cerr << "\rLUT44   ";
    Ciphertext gin44 = 1 * gout40[0] + 2 * gout33[2] + 4 * gout33[1];
    std::vector<Ciphertext> gout44 = ctx.eval_lut_amortized(&gin44, lut44);

    std::cerr << "\r          \r";
    std::vector<long> test_out;
    test_out.push_back(ctx.decrypt(&gout28[0])); /* out0 */
    test_out.push_back(ctx.decrypt(&gout34[1])); /* out1 */
    test_out.push_back(ctx.decrypt(&gout34[0])); /* out2 */
    test_out.push_back(ctx.decrypt(&gout41[0])); /* out3 */
    test_out.push_back(ctx.decrypt(&gout43[0])); /* out4 */
    test_out.push_back(ctx.decrypt(&gout42[0])); /* out5 */
    test_out.push_back(ctx.decrypt(&gout38[0])); /* out6 */
