...
std::vector<long>
test_relu_lut2fa(FHEContext& ctx, uint16_t x = 0)
{
    ...
    std::cerr << "\rLUT1   ";
    Ciphertext gin1 = 1 * ct_x9 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT2   ";
    Ciphertext gin2 = 1 * ct_x0 + 2 * gout0[0];
    ...
    auto lut3idx0 = [](long I) -> long {
        /* GATE 13 (LUT2 _45_ INIT 0x8 PERM 10) */
        switch (I) {
            case  0: return 0;
            case  1: return 0;
            case  2: return 0;
    ...
    std::cerr << "\rLUT3   ";
    Ciphertext gin3 = 1 * ct_x11 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT4   ";
    Ciphertext gin4 = 1 * gout0[0] + 2 * ct_x13;
    ...
    std::cerr << "\rLUT5   ";
    Ciphertext gin5 = 1 * ct_x4 + 2 * gout0[0];
    ...
    auto lut6idx0 = [](long I) -> long {
        /* GATE 4 (LUT2 _36_ INIT 0x8 PERM 10) */
        switch (I) {
            case  0: return 0;
            case  1: return 0;
            case  2: return 0;
    ...
    std::cerr << "\rLUT6   ";
    Ciphertext gin6 = 1 * ct_x2 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT7   ";
    Ciphertext gin7 = 1 * ct_x1 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT8   ";
    Ciphertext gin8 = 1 * ct_x7 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT9   ";
    Ciphertext gin9 = 1 * ct_x5 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT10   ";
    Ciphertext gin10 = 1 * ct_x10 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT11   ";
    Ciphertext gin11 = 1 * ct_x12 + 2 * gout0[0];
    ...
    auto lut12idx0 = [](long I) -> long {
        /* GATE 16 (LUT2 _48_ INIT 0x4 PERM 10) */
        switch (I) {
            case  0: return 0;
            case  1: return 1;
            case  2: return 0;
    ...
    std::cerr << "\rLUT12   ";
    Ciphertext gin12 = 1 * ct_x14 + 2 * ct_x15;
    ...
    std::cerr << "\rLUT13   ";
    Ciphertext gin13 = 1 * ct_x3 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT14   ";
    Ciphertext gin14 = 1 * ct_x6 + 2 * gout0[0];
    ...
    std::cerr << "\rLUT15   ";
    Ciphertext gin15 = 1 * ct_x8 + 2 * gout0[0];
    std::vector<Ciphertext> gout15 = ctx.eval_lut_amortized(&gin15, lut15);

    std::cerr << "\r          \r";
    std::vector<long> test_out;
    test_out.push_back(ctx.decrypt(&gout2[0])); /* out0 */
    test_out.push_back(ctx.decrypt(&gout7[0])); /* out1 */
    test_out.push_back(ctx.decrypt(&gout6[0])); /* out2 */
    test_out.push_back(ctx.decrypt(&gout13[0])); /* out3 */
    test_out.push_back(ctx.decrypt(&gout5[0])); /* out4 */
    test_out.push_back(ctx.decrypt(&gout9[0])); /* out5 */
    test_out.push_back(ctx.decrypt(&gout14[0])); /* out6 */
    test_out.push_back(ctx.decrypt(&gout8[0])); /* out7 */
    test_out.push_back(ctx.decrypt(&gout15[0])); /* out8 */
    test_out.push_back(ctx.decrypt(&gout1[0])); /* out9 */
    test_out.push_back(ctx.decrypt(&gout10[0])); /* out10 */
    test_out.push_back(ctx.decrypt(&gout3[0])); /* out11 */
    test_out.push_back(ctx.decrypt(&gout11[0])); /* out12 */
    test_out.push_back(ctx.decrypt(&gout4[0])); /* out13 */
    test_out.push_back(ctx.decrypt(&gout12[0])); /* out14 */
    return test_out;
}

static void
BM_relu_lut2fa(benchmark::State& state)
{
    FHEContext ctx;
    ...
    for (auto _ : state) {
        test_relu_lut2fa(ctx);
    }
}

BENCHMARK(BM_relu_lut2fa)->Unit(benchmark::kSecond);
BENCHMARK_MAIN();
