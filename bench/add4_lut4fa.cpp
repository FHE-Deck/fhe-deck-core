...
std::vector<long>
test_add4_lut4fa(FHEContext& ctx, uint8_t a = 0, uint8_t b = 0)
{
    ...
    std::cerr << "\rFA1    ";
    Ciphertext gin1 = gout0[3] + 1 * ct_a3 + 1 * ct_b3;
...
static void
BM_add4_lut4fa(benchmark::State& state)
{
    FHEContext ctx;
    ...
    for (auto _ : state) {
        test_add4_lut4fa(ctx);
    }
}

BENCHMARK(BM_add4_lut4fa)->Unit(benchmark::kSecond);
BENCHMARK_MAIN();
