...
test_image_ricker_lut3fa(FHEContext& ctx, std::vector<uint64_t> window = { 0 })
{
    ...
    std::cerr << "\rFA2    ";
    Ciphertext gin2 = gout0[2] + 1 * ct_window18 + 2 * ct_window19 + 1 * ct_window26 + 2 * ct_window27;
    std::vector<Ciphertext> gout2 = ctx.eval_lut_amortized(&gin2, decomp);

    std::cerr << "\rFA3    ";
    Ciphertext gin3 = gout2[2] + 1 * ct_window20 + 2 * ct_window21 + 1 * ct_window28 + 2 * ct_window29;
    std::vector<Ciphertext> gout3 = ctx.eval_lut_amortized(&gin3, decomp);

    std::cerr << "\rFA4    ";
    Ciphertext gin4 = 0 + 1 * ct_window0 + 2 * ct_window1 + 1 * ct_window8 + 2 * ct_window9;
    std::vector<Ciphertext> gout4 = ctx.eval_lut_amortized(&gin4, decomp);

    std::cerr << "\rFA5    ";
    Ciphertext gin5 = gout3[2] + 1 * ct_window22 + 2 * ct_window23 + 1 * ct_window30 + 2 * ct_window31;
    std::vector<Ciphertext> gout5 = ctx.eval_lut_amortized(&gin5, decomp);

    std::cerr << "\rFA6    ";
    Ciphertext gin6 = gout1[2] + 1 * ct_window42 + 2 * ct_window43 + 1 * ct_window50 + 2 * ct_window51;
    std::vector<Ciphertext> gout6 = ctx.eval_lut_amortized(&gin6, decomp);

    std::cerr << "\rFA7    ";
    Ciphertext gin7 = gout6[2] + 1 * ct_window44 + 2 * ct_window45 + 1 * ct_window52 + 2 * ct_window53;
    std::vector<Ciphertext> gout7 = ctx.eval_lut_amortized(&gin7, decomp);

    std::cerr << "\rFA8    ";
    Ciphertext gin8 = 0 + 1 * ct_window56 + 2 * ct_window57 + 1 * ct_window64 + 2 * ct_window65;
    std::vector<Ciphertext> gout8 = ctx.eval_lut_amortized(&gin8, decomp);

    std::cerr << "\rFA9    ";
    Ciphertext gin9 = gout4[2] + 1 * ct_window2 + 2 * ct_window3 + 1 * ct_window10 + 2 * ct_window11;
    std::vector<Ciphertext> gout9 = ctx.eval_lut_amortized(&gin9, decomp);

    std::cerr << "\rFA10    ";
    Ciphertext gin10 = gout7[2] + 1 * ct_window46 + 2 * ct_window47 + 1 * ct_window54 + 2 * ct_window55;
    std::vector<Ciphertext> gout10 = ctx.eval_lut_amortized(&gin10, decomp);

    std::cerr << "\rFA11    ";
    Ciphertext gin11 = gout9[2] + 1 * ct_window4 + 2 * ct_window5 + 1 * ct_window12 + 2 * ct_window13;
    std::vector<Ciphertext> gout11 = ctx.eval_lut_amortized(&gin11, decomp);

    std::cerr << "\rFA12    ";
    Ciphertext gin12 = gout8[2] + 1 * ct_window58 + 2 * ct_window59 + 1 * ct_window66 + 2 * ct_window67;
    std::vector<Ciphertext> gout12 = ctx.eval_lut_amortized(&gin12, decomp);

    std::cerr << "\rFA13    ";
    Ciphertext gin13 = 0 + 1 * gout4[0] + 2 * gout4[1] + 1 * gout0[0] + 2 * gout0[1];
    std::vector<Ciphertext> gout13 = ctx.eval_lut_amortized(&gin13, decomp);

    std::cerr << "\rFA14    ";
    Ciphertext gin14 = gout12[2] + 1 * ct_window60 + 2 * ct_window61 + 1 * ct_window68 + 2 * ct_window69;
    std::vector<Ciphertext> gout14 = ctx.eval_lut_amortized(&gin14, decomp);

    std::cerr << "\rFA15    ";
    Ciphertext gin15 = gout13[2] + 1 * gout9[0] + 2 * gout9[1] + 1 * gout2[0] + 2 * gout2[1];
    std::vector<Ciphertext> gout15 = ctx.eval_lut_amortized(&gin15, decomp);

    std::cerr << "\rFA16    ";
    Ciphertext gin16 = 0 + 1 * gout1[0] + 2 * gout1[1] + 1 * gout8[0] + 2 * gout8[1];
    std::vector<Ciphertext> gout16 = ctx.eval_lut_amortized(&gin16, decomp);

    std::cerr << "\rFA17    ";
    Ciphertext gin17 = 0 + 1 * gout13[0] + 2 * gout13[1] + 1 * gout16[0] + 2 * gout16[1];
    std::vector<Ciphertext> gout17 = ctx.eval_lut_amortized(&gin17, decomp);

    std::cerr << "\rFA18    ";
    Ciphertext gin18 = gout14[2] + 1 * ct_window62 + 2 * ct_window63 + 1 * ct_window70 + 2 * ct_window71;
    std::vector<Ciphertext> gout18 = ctx.eval_lut_amortized(&gin18, decomp);

    std::cerr << "\rFA19    ";
    Ciphertext gin19 = gout11[2] + 1 * ct_window6 + 2 * ct_window7 + 1 * ct_window14 + 2 * ct_window15;
    std::vector<Ciphertext> gout19 = ctx.eval_lut_amortized(&gin19, decomp);

    std::cerr << "\rFA20    ";
    Ciphertext gin20 = gout16[2] + 1 * gout6[0] + 2 * gout6[1] + 1 * gout12[0] + 2 * gout12[1];
    std::vector<Ciphertext> gout20 = ctx.eval_lut_amortized(&gin20, decomp);

    std::cerr << "\rFA21    ";
    Ciphertext gin21 = gout20[2] + 1 * gout7[0] + 2 * gout7[1] + 1 * gout14[0] + 2 * gout14[1];
    std::vector<Ciphertext> gout21 = ctx.eval_lut_amortized(&gin21, decomp);

    std::cerr << "\rFA22    ";
    Ciphertext gin22 = gout15[2] + 1 * gout11[0] + 2 * gout11[1] + 1 * gout3[0] + 2 * gout3[1];
    std::vector<Ciphertext> gout22 = ctx.eval_lut_amortized(&gin22, decomp);

    std::cerr << "\rFA23    ";
    Ciphertext gin23 = gout17[2] + 1 * gout15[0] + 2 * gout15[1] + 1 * gout20[0] + 2 * gout20[1];
    std::vector<Ciphertext> gout23 = ctx.eval_lut_amortized(&gin23, decomp);

    std::cerr << "\rFA24    ";
    Ciphertext gin24 = gout23[2] + 1 * gout22[0] + 2 * gout22[1] + 1 * gout21[0] + 2 * gout21[1];
    std::vector<Ciphertext> gout24 = ctx.eval_lut_amortized(&gin24, decomp);

    std::cerr << "\rINV25   ";
    std::vector<Ciphertext> gout25;
    gout25.push_back(1 - gout24[0]);

    std::cerr << "\rFA26    ";
    Ciphertext gin26 = gout21[2] + 1 * gout10[0] + 2 * gout10[1] + 1 * gout18[0] + 2 * gout18[1];
    std::vector<Ciphertext> gout26 = ctx.eval_lut_amortized(&gin26, decomp);

    std::cerr << "\rINV27   ";
    std::vector<Ciphertext> gout27;
    gout27.push_back(1 - gout24[1]);

    std::cerr << "\rFA28    ";
    Ciphertext gin28 = gout26[2] + 1 * gout10[2] + 1 * gout18[2];
    std::vector<Ciphertext> gout28 = ctx.eval_lut_amortized(&gin28, decomp);

    std::cerr << "\rFA29    ";
    Ciphertext gin29 = gout22[2] + 1 * gout19[0] + 2 * gout19[1] + 1 * gout5[0] + 2 * gout5[1];
    std::vector<Ciphertext> gout29 = ctx.eval_lut_amortized(&gin29, decomp);

    std::cerr << "\rINV30   ";
    std::vector<Ciphertext> gout30;
    gout30.push_back(1 - gout23[1]);

    std::cerr << "\rFA31    ";
    Ciphertext gin31 = gout29[2] + 1 * gout19[2] + 1 * gout5[2];
    std::vector<Ciphertext> gout31 = ctx.eval_lut_amortized(&gin31, decomp);

    std::cerr << "\rFA32    ";
    Ciphertext gin32 = gout24[2] + 1 * gout29[0] + 2 * gout29[1] + 1 * gout26[0] + 2 * gout26[1];
    std::vector<Ciphertext> gout32 = ctx.eval_lut_amortized(&gin32, decomp);

    std::cerr << "\rINV33   ";
    std::vector<Ciphertext> gout33;
    gout33.push_back(1 - gout32[0]);

    std::cerr << "\rFA34    ";
    Ciphertext gin34 = gout32[2] + 1 * gout31[0] + 2 * gout31[1] + 1 * gout28[0] + 2 * gout28[1];
    std::vector<Ciphertext> gout34 = ctx.eval_lut_amortized(&gin34, decomp);

    std::cerr << "\rINV35   ";
    std::vector<Ciphertext> gout35;
    gout35.push_back(1 - gout32[1]);

    std::cerr << "\rFA36    ";
    Ciphertext gin36 = 1 + 1 * ct_window32 + 2 * ct_window33 + 1 * gout30[0] + 2 * gout25[0];
    std::vector<Ciphertext> gout36 = ctx.eval_lut_amortized(&gin36, decomp);

    std::cerr << "\rINV37   ";
    std::vector<Ciphertext> gout37;
    gout37.push_back(1 - gout34[1]);

    std::cerr << "\rFA38    ";
    Ciphertext gin38 = gout36[2] + 1 * ct_window34 + 2 * ct_window35 + 1 * gout27[0] + 2 * gout33[0];
    std::vector<Ciphertext> gout38 = ctx.eval_lut_amortized(&gin38, decomp);

    std::cerr << "\rINV39   ";
    std::vector<Ciphertext> gout39;
    gout39.push_back(1 - gout34[2]);
    ...
    std::cerr << "\rLUT40   ";
    Ciphertext gin40 = 1 * gout36[1] + 2 * gout36[0];
    ...
    std::cerr << "\rLUT41   ";
    Ciphertext gin41 = 1 * gout36[1] + 2 * gout36[0] + 4 * gout38[0];
    std::vector<Ciphertext> gout41 = ctx.eval_lut_amortized(&gin41, lut41);

    std::cerr << "\rINV42   ";
    std::vector<Ciphertext> gout42;
    gout42.push_back(1 - gout34[0]);

    std::cerr << "\rFA43    ";
    Ciphertext gin43 = gout38[2] + 1 * ct_window36 + 2 * ct_window37 + 1 * gout35[0] + 2 * gout42[0];
    std::vector<Ciphertext> gout43 = ctx.eval_lut_amortized(&gin43, decomp);

    std::cerr << "\rFA44    ";
    Ciphertext gin44 = gout43[2] + 1 * ct_window38 + 2 * ct_window39 + 1 * gout37[0] + 2 * gout39[0];
    ...
    std::cerr << "\rLUT45   ";
    Ciphertext gin45 = 1 * gout41[0] + 2 * gout38[1];
    ...
    std::cerr << "\rLUT46   ";
    Ciphertext gin46 = 1 * gout41[0] + 2 * gout38[1] + 4 * gout44[2];
    ...
    std::cerr << "\rLUT47   ";
    Ciphertext gin47 = 1 * gout36[1] + 2 * gout36[0] + 4 * gout44[2];
    ...
    std::cerr << "\rLUT48   ";
    Ciphertext gin48 = 1 * gout40[0] + 2 * gout38[0] + 4 * gout44[2];
    ...
    std::cerr << "\rLUT49   ";
    Ciphertext gin49 = 1 * gout45[0] + 2 * gout43[0] + 4 * gout44[2];
    ...
    std::cerr << "\rLUT50   ";
    Ciphertext gin50 = 1 * gout43[0] + 2 * gout41[0] + 4 * gout38[1];
    ...
    std::cerr << "\rLUT51   ";
    Ciphertext gin51 = 1 * gout50[0] + 2 * gout43[1] + 4 * gout44[2];
    ...
    std::cerr << "\rLUT52   ";
    Ciphertext gin52 = 1 * gout50[0] + 2 * gout43[1] + 4 * gout44[0];
    ...
    std::cerr << "\rLUT53   ";
    Ciphertext gin53 = 1 * gout52[0] + 2 * gout44[2] + 4 * gout44[1];
    ...
    test_out.push_back(ctx.decrypt(&gout48[0])); /* out2 */
    test_out.push_back(ctx.decrypt(&gout46[0])); /* out3 */
    test_out.push_back(ctx.decrypt(&gout49[0])); /* out4 */
