...
std::vector<long>
test_relu_lut2(FHEContext& ctx, uint16_t x = 0)
{
    ...
    std::cerr << "\rLUT11   ";
    Ciphertext gin11 = 1 * ct_x11 + 2 * gout1[0];
    ...
    std::cerr << "\rLUT14   ";
    Ciphertext gin14 = 1 * ct_x2 + 2 * gout1[0];
...
static void
BM_relu_lut2(benchmark::State& state)
{
    FHEContext ctx;
...
BENCHMARK(BM_relu_lut2)->Unit(benchmark::kSecond);
BENCHMARK_MAIN();
