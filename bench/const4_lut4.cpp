...
std::vector<long>
test_const4_lut4(FHEContext& ctx, uint8_t a = 0)
{
    ...
    test_out.push_back(ctx.decrypt(&gout0[1])); /* out3 */
    test_out.push_back(ctx.decrypt(&gout0[0])); /* out4 */
    return test_out;
}

static void
BM_const4_lut4(benchmark::State& state)
{
    FHEContext ctx;
    ...
    for (auto _ : state) {
        test_const4_lut4(ctx);
    }
}

BENCHMARK(BM_const4_lut4)->Unit(benchmark::kSecond);
BENCHMARK_MAIN();
