...
std::vector<long>
test_add4_lut2fa(FHEContext& ctx, uint8_t a = 0, uint8_t b = 0)
{
...
static void
BM_add4_lut2fa(benchmark::State& state)
{
    FHEContext ctx;
    ...
    for (auto _ : state) {
        test_add4_lut2fa(ctx);
    }
}

BENCHMARK(BM_add4_lut2fa)->Unit(benchmark::kSecond);
BENCHMARK_MAIN();
