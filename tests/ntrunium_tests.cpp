...
/** @brief Comment ends at the end of the file.
 * Uncomment if you want to integrade the matrix inversion examples.
 * But note that actually for example inversion modulo or other operation
 * can be implemented much easier now with the new interface.
...
void test_key_switching(ntrunium_named_param param_name){
    ...
    Sampler rand; 
    int t = 5;
    PlaintextEncoding encoding(full_domain, t, param_gen.ntru.param.Q);
    ...
    long dec = param_gen.lwe_g.lwe->decrypt(lwe_ct, encoding);
...
void rotating_ntru_test(){
    ...
    LWESK lwe(param, lwe_stddev, binary);
    long t = 3; 
    ...
    long ct_msg = 0;  
    LWECT *ct = lwe.encrypt(ct_msg);  
    long phase = lwe.partial_decrypt(ct->ct);   

    // Test rotating NTRU LWE decryption 
    long ct_msg_test_rot = delta_q_t * 0; 
    long *ct_test_rot = ntru_par.init_poly(); 
    long phase_test_rot = lwe.partial_decrypt(ct->ct);  
    ...
    for(int j = 0; j < N; ++j){ 
        lwe.encrypt(ct->ct, j);  
        phase = lwe.partial_decrypt(ct->ct);    
        ...
        Utils::rotate_poly(acc_test_rot, acc.c, ntru_par.engine->N, ct->ct[0]);
        for(int i = 0; i < n; ++i){
            if(lwe.key[i] == 1){   
                Utils::rotate_poly(acc_test_rot_next.c, acc_test_rot, ntru_par.engine->N, ct->ct[i+1]);
            }else{ 
...
void test_blind_rotate_named_params(ntrunium_named_param param_name, int num_of_rotations){ 
    ...
    for(int i = 0; i < num_of_rotations; ++i){ 
        ...
        test_phase = param_gen.lwe_small->partial_decrypt(ct); 
        // Blind rotate  
...
void test_bootstrap_named_params(ntrunium_named_param param_name, int num_of_tests){ 
    ...
    RotationPoly acc_msg = RotationPoly::rot_msb(t, param_gen.N, param_gen.P);   
    ntru_ct acc_pot = param_gen.ntru_P.kdm_encrypt(acc_msg.coefs);    
...
void test_bootstrap_with_functional_rotation_polynomials(ntrunium_named_param param_name, int num_of_tests){
    ...
    long* acc_msg = RotationPoly::rot_identity(t, param_gen.N, param_gen.P);     
    ntru_ct acc_pot = param_gen.ntru_P.kdm_encrypt(acc_msg);   
    RotationPoly acc_msb_msg = RotationPoly::rot_msb(4, param_gen.N, param_gen.P);     
    ntru_ct acc_msb = param_gen.ntru_P.kdm_encrypt(acc_msb_msg.coefs);    
    ntru_ct ct(param_gen.ntru.param);
    long* msg = param_gen.ntru.param.init_zero_poly();
...
int main(){   
    std::cout << "Run ntrunium test" << std::endl;
