...
void general_modulus_gaussian_samping_precomputed_values_test(){
    ...
    long Q = 2048;
    int basis = 8;
    std::shared_ptr<DiscreteGaussianSamplingGadget> g =  std::shared_ptr<DiscreteGaussianSamplingGadget>(new DiscreteGaussianSamplingGadget(N, Q, basis, 16.0)); 

    std::cout << "g.digits: " << g->digits << std::endl;
    // Test l and h
    if(g->l[0] * g->l[0] + g->h[1] * g->h[1] != g->r * g->r - g->base * g->base){
        std::cout << "- Fail at: g.l[0] * g.l[0] + g.h[1] * g.h[1] != g.r * g.r - g.basis * g.basis!" << std::endl;
        std::cout << "Left: " << g->l[0] * g->l[0] + g->h[1] * g->h[1] << std::endl;
        std::cout << "Right: " <<  g->r * g->r - g->base * g->base << std::endl;
        std::cout << "g.l[0]: " << g->l[0] << std::endl;
        std::cout << "g.h[1]: " << g->h[1] << std::endl;
        std::cout << "g.r: " << g->r << std::endl;
        std::cout << "g.basis: " << g->base << std::endl;
        //return;
    }
    for(int i = 1; i < g->digits-1; ++i){
        if(g->l[i] * g->l[i] + g->h[i] * g->h[i] != g->r * g->r - (g->base * g->base - 1)){
            ...
            std::cout << "Right: " <<  g->r * g->r - (g->base * g->base - 1) << std::endl;
        ...
        if(g->l[i] * g->h[i] != g->base){
            std::cout << "- Fail at: g.l[i] * g.h[i] != g.basis! in i:" << i << std::endl;
            std::cout << "Left: " << g->l[i] * g->h[i] << std::endl;
            std::cout << "Right: " <<  g->base << std::endl;
            std::cout << "g.l[i]: " << g->l[i] << std::endl;
            std::cout << "g.h[i]: " << g->h[i] << std::endl; 
            std::cout << "g.basis: " << g->base << std::endl;
        }
    }
    if(g->l[g->digits-1] * g->l[g->digits-1]  != g->r * g->r - g->base * g->base){
        std::cout << "- Fail at: g.l[g.ell-1] * g.l[g.ell-1]  != g.r * g.r - g.basis * g.basis!" << std::endl;
        std::cout << "Left: " << g->l[g->digits-1] * g->l[g->digits-1] << std::endl;
        std::cout << "Right: " <<  g->r * g->r - g->base * g->base << std::endl;
        std::cout << "g.l[g.ell-1] : " << g->l[g->digits-1]  << std::endl; 
        std::cout << "g.r: " << g->r << std::endl;
        std::cout << "g.basis: " << g->base << std::endl;
        //return;
    }
    ...
    // test d
    if(g->d[0] *  g->base != g->q_decomp[0]){
        std::cout << "- Fail at: g.d[i] *  g.basis  != g.q_decomp[i]! in i:" << 0 << std::endl;
        std::cout << "Left: " << g->d[0] *  g->base << std::endl;
    ...
    for(int i = 1; i < g->digits-1; ++i){
        if(g->d[i] *  g->base - g->d[i-1] != g->q_decomp[i]){
            std::cout << "- Fail at: g.d[i] *  g.basis  != g.q_decomp[i]! in i:" << i << std::endl;
            std::cout << "Left: " << g->d[i] *  g->base - g->d[i-1]  << std::endl;
...
int main(){
    ...
    gadget_sampling_correctness_test(100, 2048, 4294955009, 256, 300, discrete_gaussian_gadget);
    // NTT prime smaller than 2**33
    gadget_sampling_correctness_test(100, 2048, 8589905921, 1024, 1025, discrete_gaussian_gadget);
    // NTT prime smaller than 2**36
    gadget_sampling_correctness_test(100, 2048, 68719464449, 16, 20, discrete_gaussian_gadget); 

    gadget_sampling_correctness_test(100, 2048, 35184372088831, 16, 20, discrete_gaussian_gadget); 
